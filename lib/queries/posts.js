...
const verifyPrivacityPost = async (
  ...
  console.log(post.users.id, "userId");
    ...
    const response = await isAuth(
      async (user) => {
        ...
        if (posts.length) {
                ...
                try {
                  let newPost = await verifyPrivacityPost(
                    async (post) => {
                      ...
                      const postReturned = await getPost(post);

                      console.log("postReturned", postReturned);

                      return postReturned;
                    },

                    async (post) => await getPrivatedPost(post),
          ...
          if (
            privacityUserProfile === "all" ||
            args.input.userId === myUserId
          ) {
            for (const post of posts) {
              try {
                let newPost = await verifyPrivacityPost(
