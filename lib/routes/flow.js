...
flowRoutes.post("/payment/result", async (req, res, next) => {
  ...
  if (response.status === 3) {
    ...
    let amount = await convertCLPtoUSD(
      parseFloat(response.paymentData.balance)
    );
      ...
      where: {
        customer: {
          user: {
            creator: {
              id: {
                equals: digitalProduct.creatorId,
              },
            },
          },
        },
    ...
    await addMovement(sale.customer.id, "revenue", amount, walletReceiver.id);
