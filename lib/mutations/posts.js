...
const fs = require("fs");
const emmitedNotification = require("../pusher");
    ...
    if (
      ...
      if (!likes) {
        const responsePrisma = await prisma.likes
          ...
          .then(async (data) => {
            try {
              console.log(data.creator.userId, idUser);
              if (data.creator.userId !== idUser) {
                console.log("emitir notificacion");

                await emmitedNotification(
                  data.creator.userId,
        ...
        if (responsePrisma) {
          const newLikes = responsePrisma.media.likes.filter(
    ...
    const response = await isAuth(
      async (user) => {
        ...
        const createComment = async () => {
          ...
          if (comment)
            if (comment.posts.usersId !== idUser) {
              emmitedNotification(
