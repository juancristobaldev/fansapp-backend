...
module.exports = {
  ...
  updateProfile: async (parent, args, context) => {
    const { id, description, frontPage, tiktok, instagram } = args.input;
    const errors = [];
    ...
    const response = await isAuth(
      async (user) => {
        ...
        if (newArgs.photo) newArgs.photo = JSON.parsed(newArgs.photo);

        if (newArgs.frontPage) newArgs.photo = JSON.parsed(newArgs.photo);
